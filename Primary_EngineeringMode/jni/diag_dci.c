...
static int login(int pwd, int len)
{
    int ret;
	...
	unsigned char buf[5] = {75, 68, 0, 64, 0};
	buf[4] = pwd;
	err = diag_send_dci_async_req(client_id, buf, 5, dci_rsp_pkt, 100, &process_response, NULL);
	if (err != DIAG_DCI_NO_ERROR)
...
static jboolean send_data(JNIEnv *env, jobject object, jint pwd)
{
    int ret = -1;
    int len;
    int npwd;
//    npwd = (*env)->GetStringUTFChars(env, pwd, NULL);
//    len = (*env)->GetStringUTFLength(env, pwd);
	DIAG_LOGE("hanque diag: Modem supports DCI%d\n",pwd);
    ret = login(pwd, 0);
...
{
    const char* const kClassPathName = "com/android/engineeringmode/manualtest/LTEDivAntTest";
